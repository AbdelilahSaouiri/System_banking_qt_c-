...
//Définir les données de la boîte de dialogue
...
//Charger les données dans la boîte de dialogue
...
//Bouton Retour de l'ouverture de compte
...
//Vérifier les champs vides de l'ouverture de compte

bool account_open::checknull() {
    if (ui->ao_acc_no->text().isEmpty() || ui->ao_name->text().isEmpty() || ui->ao_mobno->text().isEmpty() || ui->ao_nationality->text().isEmpty() || ui->ao_address->text().isEmpty()) {
        return true;
    }
    return false;
}
...
bool account_open::conn() {
    ...
    mydb.setDatabaseName("C:/Bank-System/accountdata.db");
    if (mydb.open()) {
        return true;
    } else {
        return false;
...
//Obtenir les données du compte d'épargne
...
//Obtenir les données du compte courant
...
//Enregistrer les données du compte d'épargne dans la base de données
...
void account_open::sa_to_database(saving_account k) {
    ...
    if (query.exec()) {
        QMessageBox::information(this, "Account", "Le compte a été créé avec succès");
...
//Enregistrer les données du compte courant dans la base de données
...
void account_open::on_ao_push_clicked() {
    if (ui->ao_push->text() == "Creer Compte") {
        if (account_open::checknull()) {
            QMessageBox::warning(this, "Account", "Veuillez entrer toutes les informations !!");
        } else {
            if (ui->ao_type->currentText() == "Un compte épargne") {
        ...
        if (account_open::checknull()) {
            ...
            if (account_open::updatedata(k)) {
                QMessageBox::information(this, "Account", "Compte mis à jour !!");
                account_open::cleardata();
                account_open::setoff();
            }
        }
...
void account_open::on_ao_acc_no_returnPressed() {
    if (obj.checkacc_no(ui->ao_acc_no->text())) {
        ...
        QMessageBox::warning(this, "Account", "Aucun compte trouvé !!");
    }
}
